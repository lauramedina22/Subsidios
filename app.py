...
# ── Sidebar ──
_opciones_nav = ["Proveedores", "Sedes", "Estudiantes", "Consumos"]
_idx_default = 0
...
# ────────── TABLA ──────────
def render_tabla(items, cols_def, id_field, servicio, lookup=None, seccion_key=""):
    if not items:
    ...
    def fmt(item, attr):
        ...
        if lookup and attr in lookup:
            ...
            val = str(val)
        if is_bool:
            bc = "badge-active" if val == "Sí" else "badge-inactive"
...
def form_proveedor():
    ...
    with st.form("form_proveedor"):
        st.markdown(f'<h3 style="color:#0B2545;font-family:\'Source Serif 4\',serif;font-size:1.1rem;font-weight:600;margin-bottom:12px;">{tit}</h3>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
        ...
        cols = st.columns(2)
        if cols[0].form_submit_button("Guardar" if st.session_state.modo_form == "crear" else "Actualizar", type="primary"):
...
def form_sede(proveedores_lista):
    ...
    with st.form("form_sede"):
        ...
        with c2:
            ...
            prov_id = st.selectbox("Proveedor", options=prov_keys,
                                   format_func=lambda x: prov_opts[x],
                                   index=default_idx) if prov_opts else None
        cols = st.columns(2)
        if cols[0].form_submit_button("Guardar" if st.session_state.modo_form == "crear" else "Actualizar", type="primary"):
            if prov_id:
                if st.session_state.modo_form == "editar":
...
def form_estudiante():
    ...
    with st.form("form_estudiante"):
        ...
        with c2:
            ...
            tipo = st.selectbox("Tipo almuerzo", ["Carnívoro", "Vegetariano"],
        ...
        if cols[0].form_submit_button("Guardar" if st.session_state.modo_form == "crear" else "Actualizar", type="primary"):
            if st.session_state.modo_form == "editar":
                ...
                e = Estudiante(codigo, nombre_completo, correo_est, activo_est,
...
def form_consumo(estudiantes_lista, sedes_lista):
    ...
    with st.form("form_consumo"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            est_id_key = st.selectbox("Estudiante", options=list(opts_est.keys()),
                                      format_func=lambda x: opts_est[x]) if opts_est else None
            sede_id_key = st.selectbox("Sede", options=list(opts_sede.keys()),
                                       format_func=lambda x: opts_sede[x]) if opts_sede else None
        with c2:
        ...
        if cols[0].form_submit_button("Guardar" if st.session_state.modo_form == "crear" else "Actualizar", type="primary"):
            if est_id_key and sede_id_key:
                if st.session_state.modo_form == "editar":
                    ...
                    c = Consumo(ObjectId(est_id_key), ObjectId(sede_id_key),
...
def seccion_proveedores():
    todos = proveedor_svc.obtener_todos()
    st.markdown('<h2 style="margin-bottom:4px;">Proveedores</h2>', unsafe_allow_html=True)
    m1, m2, m3 = st.columns(3)
    with m1: render_stat_card("truck", len(todos), "Total proveedores", "#1B4079")
    with m2: render_stat_card("check-circle", sum(1 for p in todos if p.estado_activo), "Activos", "#C9A227")
    with m3: render_stat_card("ban", sum(1 for p in todos if not p.estado_activo), "Inactivos", "#5C6470")
...
def seccion_sedes():
    ...
    with m2: render_stat_card("users", sum(s.capacidad_maxima for s in todos), "Capacidad total", "#C9A227")
...
def seccion_estudiantes():
    todos = estudiante_svc.obtener_todos()
...
def seccion_consumos():
    ...
    with m2: render_stat_card("check-circle", sum(1 for c in todos if c.validacion_identidad), "Validados", "#C9A227")
